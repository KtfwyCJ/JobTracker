...
export default function TipDetail({ tip, onDeleted }: Props) {
  ...
  const [guideMode, setGuideMode] = useState<'read' | 'edit'>('read')
  ...
  // Guide auto-save
  const [guide, setGuide] = useState(tip.guide ?? '')
  ...
  const saveGuide = useCallback(
    (value: string) => {
      ...
      guideDebounce.current = setTimeout(() => {
        updateInterviewTip(tip.id, { guide: value })
      }, 400)
    },
    [tip.id, updateInterviewTip]
  )
    ...
    <div className="flex h-full w-full flex-col overflow-hidden bg-white">
      ...
      <div className="flex shrink-0 items-center justify-between border-b border-zinc-200 px-4 pt-3">
        ...
        <div className="flex items-center gap-2 pb-2">
          {tab === 'guide' && (
            <button
              onClick={() => setGuideMode(guideMode === 'read' ? 'edit' : 'read')}
              className="rounded-md border border-zinc-200 px-2.5 py-1 text-xs font-medium text-zinc-600 hover:bg-zinc-50"
            >
              {guideMode === 'read' ? 'Edit' : 'Preview'}
            </button>
          )}
          <button
            onClick={handleDelete}
            className="rounded-md border border-red-200 bg-red-50 px-2.5 py-1 text-xs font-medium text-red-500 hover:bg-red-100"
          >
            Delete
          </button>
        </div>
      ...
      <div className="shrink-0 border-b border-zinc-200 px-4 py-2">
        ...
        <div className="flex items-center gap-1.5 text-xs text-zinc-500">
          ...
          {linkedJob && (
            <>
              <span>·</span>
              <button onClick={handleGoToJob} className="text-blue-500 hover:underline">
          ...
          <section>
            ...
            {tip.questions.length > 0 && (
              <div className="mb-2 flex max-h-44 flex-col gap-1 overflow-y-auto">
                {tip.questions.map((q) => (
                  <div key={q.id} className="flex items-start gap-2 rounded-md bg-zinc-50 px-2.5 py-1.5">
                    {q.kind === 'link' ? (
                    ...
                    <button onClick={() => deleteTipQuestion(tip.id, q.id)}
                      className="shrink-0 text-[10px] text-zinc-300 hover:text-zinc-500">✕</button>
                  </div>
                ))}
              </div>
            )}
              ...
              <div className="mb-2 flex flex-col gap-1">
                {tip.sources.map((s) => (
                  <div key={s.id} className="flex items-center gap-2 rounded-md bg-zinc-50 px-2.5 py-1.5">
                    ...
                    <button onClick={() => deleteTipSource(tip.id, s.id)}
                      className="shrink-0 text-[10px] text-zinc-300 hover:text-zinc-500">✕</button>
                  </div>
                ))}
              </div>
            ...
            <div className="flex flex-col gap-1">
              {tip.checklist.map((item) => (
                <div key={item.id}
                  ...
                  <button onClick={() => toggleTipItem(tip.id, item.id)}
                    className={`shrink-0 text-xs ${item.done ? 'text-green-500' : 'text-zinc-300 hover:text-zinc-500'}`}>
                    {item.done ? '✓' : '○'}
                  </button>
                  <span className={`flex-1 text-xs ${item.done ? 'text-zinc-400 line-through' : 'text-zinc-900'}`}>
                    {item.text}
                  </span>
                  <button onClick={() => deleteTipItem(tip.id, item.id)}
                    className="shrink-0 text-[10px] text-zinc-200 hover:text-zinc-400">✕</button>
                </div>
              ))}
              {showAddItem ? (
                <div className="flex gap-1.5">
        ...
        <div className="flex flex-1 flex-col overflow-hidden">
          {guideMode === 'edit' ? (
            <textarea
              ...
              placeholder="Paste your markdown prep guide here…"
              className="flex-1 resize-none p-4 font-mono text-xs text-zinc-700 outline-none"
            />
          ) : (
            <div className="flex-1 overflow-y-auto p-4">
              {guide.trim() ? (
                <div className="prose prose-sm max-w-none prose-headings:text-zinc-900 prose-h1:text-lg prose-h2:text-base prose-h3:text-sm prose-p:text-zinc-700 prose-li:text-zinc-700 prose-a:text-blue-500 prose-strong:text-zinc-900 prose-hr:border-zinc-200 prose-blockquote:border-zinc-300 prose-blockquote:text-zinc-500">
                  <ReactMarkdown remarkPlugins={[remarkGfm]}>{guide}</ReactMarkdown>
                </div>
              ) : (
                <div className="flex h-full items-center justify-center">
                  <div className="text-center">
                    <p className="text-sm text-zinc-400">No guide yet.</p>
                    <button
                      onClick={() => setGuideMode('edit')}
                      className="mt-2 text-sm text-blue-500 hover:underline"
                    >
                      Click Edit to paste your prep document
                    </button>
                  </div>
                </div>
              )}
